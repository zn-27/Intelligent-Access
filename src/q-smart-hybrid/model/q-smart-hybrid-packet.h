...
#ifndef Q_SMART_HYBRID_PACKET_H
#define Q_SMART_HYBRID_PACKET_H
...
enum MessageType
{
  QSHHYBRIDTYPE_RREQ = 1,      ///< Route Request
  QSHHYBRIDTYPE_RREP = 2,      ///< Route Reply
  QSHHYBRIDTYPE_RERR = 3,      ///< Route Error
  QSHHYBRIDTYPE_RREP_ACK = 4   ///< Route Reply Acknowledgment
};

/**
 * \ingroup qSmartHybrid
 * \brief AODV types header
 */
...
public:
   ...
   */
  TypeHeader (MessageType t = QSHHYBRIDTYPE_RREQ);
 ...
 *
 * Based on AODV RREQ with additional Smart-AODV minSnr field.
 */
...
public:
  ...
  // Smart-AODV: Path quality metric
  /**
   * \brief Set the minimum SNR along the path
   * \param snr the minimum SNR value in dB
   */
...
private:
  uint8_t m_flags;         ///< |J|R|G|D|U| bit flags
  ...
  double m_minSnr;         ///< Minimum SNR along the path (dB)
};

/**
 * \brief Stream output operator
 * \param os output stream
 * \param h the RreqHeader
 * \return updated stream
 */
...
public:
  ...
  /**
   * \brief Configure RREP to be a Hello message
...
public:
  ...
  /**
   * \brief Add unreachable node address and its sequence number
   * \param dst unreachable IPv4 address
   * \param seqNo unreachable sequence number
   * \return false if we already added maximum number of unreachable destinations
   */
  bool AddUnDestination (Ipv4Address dst, uint32_t seqNo);

  /**
   * \brief Delete pair (address + sequence number) from RERR header
   * \param un unreachable pair
  ...
  /**
   * \brief Get number of unreachable destinations

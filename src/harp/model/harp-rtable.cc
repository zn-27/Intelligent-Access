...
NS_LOG_COMPONENT_DEFINE ("HarpRoutingTable");
...
RoutingTableEntry::RoutingTableEntry (Ptr<NetDevice> dev, Ipv4Address dst, bool vSeqNo, uint32_t seqNo,
                                       Ipv4InterfaceAddress iface, double metric, Ipv4Address nextHop, Time lifetime,
                                       RouteOrigin origin)
...
RoutingTable::Update (RoutingTableEntry & rt)
{
  ...
  RouteOrigin newOrigin = rt.GetOrigin ();
  RouteOrigin existingOrigin = i->second.GetOrigin ();
  double newMetric = rt.GetMetric ();
  double existingMetric = i->second.GetMetric ();

  bool shouldUpdate = true;

  if (newOrigin == ORIGIN_PROACTIVE && existingOrigin == ORIGIN_REACTIVE)
    {
      if (newMetric >= existingMetric * 1.1)
        {
          NS_LOG_LOGIC ("Proactive update to " << rt.GetDestination () << " rejected; existing reactive metric is significantly better");
          shouldUpdate = false;
        }
    }
...
void
RoutingTable::InvalidateRoutesWithNextHop (Ipv4Address nextHop)
{
  ...
  for (std::map<Ipv4Address, RoutingTableEntry>::iterator i =
         m_ipv4AddressEntry.begin (); i != m_ipv4AddressEntry.end (); ++i)
    {
      if ((i->second.GetNextHop () == nextHop) && (i->second.GetFlag () == VALID))
        {
          NS_LOG_LOGIC ("Invalidate route with next hop " << nextHop << " to " << i->first);
          i->second.Invalidate (m_badLinkLifetime);
        }
    }
}

 ...
 */
#ifndef HARP_RTABLE_H
#define HARP_RTABLE_H
...
/**
 * \ingroup harp
 * \brief Route origin types
 */
enum RouteOrigin
{
  ORIGIN_REACTIVE,     //!< Derived from RREQ/RREP
  ORIGIN_PROACTIVE     //!< Derived from Hello/TC
};
...
public:
  ...
  /**
   * Get the route origin
   * \returns route origin type
   */
 ...
 private:
  ...
  uint32_t m_seqNo;
  /// Metric (replaces hop count)
  double m_metric;
  /// Route origin (Reactive vs Proactive)
...
/**
 * \ingroup harp
 * \brief The Routing table used by HARP protocol
 */
class RoutingTable
{
public:
  ...
  /**
   * Invalidate routes with specified next hop

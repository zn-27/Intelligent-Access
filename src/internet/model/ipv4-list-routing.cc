...
//扩展了一个可以动态修改优先级别的函数
/**
 * @brief Modify the priority of a routing protocol at a given index.
 * 根据序号修改路由协议优先级
 * @param index Index of the routing protocol in the list
 * @param newPriority New priority value
 */
//
void
Ipv4ListRouting::SetRoutingProtocolPriority(uint32_t index, int16_t newPriority)
{
    NS_LOG_FUNCTION(this << index << newPriority);

    if (index >= m_routingProtocols.size())
    {
        NS_FATAL_ERROR("Ipv4ListRouting::SetRoutingProtocolPriority(): index " << index
                                                                               << " out of range");
    }

    uint32_t i = 0;
    for (auto rprotoIter = m_routingProtocols.begin(); rprotoIter != m_routingProtocols.end(); rprotoIter++, i++)
    {
        if (i == index)
        {
            // 修改优先级
            rprotoIter->first = newPriority;
            break;
        }
    }
...
/**
 * @brief Modify the priority of a routing protocol based on its type.
 * 根据typeID修改路由协议优先级
 * @param type Protocol TypeId to match
 * @param newPriority New priority value
 * @return true if a protocol was found and updated, false otherwise
 */
bool
Ipv4ListRouting::SetRoutingProtocolPriorityByType(TypeId type, int16_t newPriority)
{
    NS_LOG_FUNCTION(this << type << newPriority);
    bool updated = false;

    for (auto rprotoIter = m_routingProtocols.begin(); rprotoIter != m_routingProtocols.end(); rprotoIter++)
    {
        if ((*rprotoIter).second->GetInstanceTypeId() == type)
        {
            rprotoIter->first = newPriority;
            updated = true;
            // 如果有多个相同类型的协议需要全部修改，可以去掉 break
            break;
        }
    }

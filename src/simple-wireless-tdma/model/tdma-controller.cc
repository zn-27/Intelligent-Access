...
TdmaController::ScheduleTdmaSession (const uint32_t slotNum)
{
  ...
  if (it == m_slotPtrs.end ())
    {
      // Skip empty slots - no MAC registered for this slot
      NS_LOG_DEBUG ("No MAC for slot " << slotNum << ", skipping to next slot");
      if (slotNum + 1 >= GetTotalSlotsAllowed ())
        {
          NS_LOG_DEBUG ("Reached end of frame, starting over");
          Simulator::Schedule (GetInterFrameTimeInterval (), &TdmaController::StartTdmaSessions, this);
        }
      else
        {
          // Schedule next slot immediately (no transmission time for empty slot)
          Simulator::Schedule (MicroSeconds (1), &TdmaController::ScheduleTdmaSession, this, slotNum + 1);
        }
      return;
    }

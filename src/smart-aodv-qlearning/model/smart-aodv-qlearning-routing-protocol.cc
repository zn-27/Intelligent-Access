...
#include "smart-aodv-qlearning-routing-protocol.h"
#include "ns3/log.h"
...
namespace ns3
{
  ...
  NS_LOG_COMPONENT_DEFINE("SmartAodvQlearningRoutingProtocol");

  namespace smartAodvQlearning
  {
    ...
    public:
      ...
      {
        static TypeId tid = TypeId("ns3::smartAodvQlearning::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Aodv")
    ...
    {
      static TypeId tid = TypeId("ns3::smartAodvQlearning::RoutingProtocol")
    ...
    RoutingProtocol::SendRequest(Ipv4Address dst)
    {
      ...
      // SmartAodv: Set SNR field to current best known link quality
      // For RREQ originating from this node, we don't have measured SNR yet,
      // so set to 0 (unknown). Will be updated by intermediate nodes.
      rreqHeader.SetMinSnr(0.0);
    ...
    RoutingProtocol::UpdateRouteToNeighbor(Ipv4Address sender, Ipv4Address receiver)
    {
      ...
      if (!m_routingTable.LookupRoute(sender, toNeighbor))
      {
        ...
        // SmartAodv: Initialize with default RSSI value
        newEntry.SetLastRssi(-70.0); // Default reasonable RSSI
        newEntry.SetMinSnr(25.0);    // Default SNR
        newEntry.SetLinkExpiryTime(PredictLinkExpiry(-70.0));
        m_routingTable.AddRoute(newEntry);
      }
      else
      {
        ...
        if (toNeighbor.GetValidSeqNo() && (toNeighbor.GetHop() == 1) && (toNeighbor.GetOutputDevice() == dev))
        {
          ...
          // SmartAodv: Update link expiry time
          toNeighbor.SetLinkExpiryTime(PredictLinkExpiry(toNeighbor.GetLastRssi()));
        }
        else
        {
          RoutingTableEntry newEntry(/*device=*/dev, /*dst=*/sender, /*know seqno=*/false, /*seqno=*/0,
          ...
          // SmartAodv: Copy existing link quality metrics
          newEntry.SetLastRssi(toNeighbor.GetLastRssi());
          newEntry.SetMinSnr(toNeighbor.GetMinSnr());
          newEntry.SetLinkExpiryTime(PredictLinkExpiry(toNeighbor.GetLastRssi()));
    ...
    RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
    {
      ...
      // SmartAodv: Update sender's RSSI in routing table
      // Simulate getting RSSI from the packet reception
      // In a real implementation, this would come from PhyRxStats callback
      RoutingTableEntry senderEntry;
      if (m_routingTable.LookupRoute(src, senderEntry))
      {
        // Simulate RSSI value (in practice, this would come from PhyRxStats)
        double simulatedRssi = -65.0 + (m_uniformRandomVariable->GetValue(-20.0, 20.0)); // -85 to -45 dBm range
        senderEntry.SetLastRssi(simulatedRssi);
        senderEntry.SetMinSnr(simulatedRssi - 95.0); // Calculate SNR with approximate noise floor
        senderEntry.SetLinkExpiryTime(PredictLinkExpiry(simulatedRssi));
        m_routingTable.Update(senderEntry);
        NS_LOG_DEBUG("SmartAodv: Updated " << src << " RSSI to " << simulatedRssi << " dB");
      }

      // SmartAodv: Check signal quality of received RREQ
      // If SNR is too low, discard the RREQ to avoid building unstable routes
      double rreqSnr = rreqHeader.GetMinSnr();
      const double SNR_THRESHOLD = 5.0; // dB - minimum acceptable SNR
      if (rreqSnr > 0 && rreqSnr < SNR_THRESHOLD)
      {
        NS_LOG_DEBUG("SmartAodv: Dropping RREQ from " << src
                                                      << " due to low SNR: " << rreqSnr << " dB");
        return;
      }
      ...
      // SmartAodv: Update RREQ with current link quality information
      // Get current RSSI for the link from sender (src)
      // Reuse senderEntry declared earlier in this function
      if (m_routingTable.LookupRoute(src, senderEntry))
      {
        // Get stored RSSI from the sender
        double currentRssi = senderEntry.GetLastRssi();
        if (currentRssi < -90.0) // Invalid RSSI value
        {
          currentRssi = -70.0; // Use a reasonable default
        }

        // Update RREQ with minimum SNR along the path
        double minSnr = rreqHeader.GetMinSnr();
        double currentSnr = currentRssi - 95.0; // Approximate noise floor

        // Take the minimum SNR along the path
        if (minSnr <= 0.0 || currentSnr < minSnr)
        {
          rreqHeader.SetMinSnr(currentSnr);
          NS_LOG_DEBUG("SmartAodv: Updated RREQ minSNR to " << currentSnr
                                                            << " dB (from " << src << ")");
        }
      }
    ...
    RoutingProtocol::SendPacketFromQueue(Ipv4Address dst, Ptr<Ipv4Route> route)
    {
      ...
      // SmartAodv: Check link expiry time for proactive route discovery
      RoutingTableEntry rtEntry;
      if (m_routingTable.LookupRoute(dst, rtEntry))
      {
        Time linkExpiry = rtEntry.GetLinkExpiryTime();
        Time routeLifetime = rtEntry.GetLifeTime();

        // If link is about to expire, send preemptive RREQ
        const Time PREEMPTIVE_THRESHOLD = Seconds(0.5);
        if (routeLifetime < PREEMPTIVE_THRESHOLD && routeLifetime > Seconds(0))
        {
          NS_LOG_INFO("SmartAodv: Route to " << dst
                                             << " expiring soon (" << routeLifetime.As(Time::S) << " s), "
                                             << "sending preemptive RREQ");
          SendPreemptiveRreq(dst);
        }
      }
    ...
    /// SmartAodv: Monitor physical layer for RSSI/SNR
    void
    RoutingProtocol::PhyRxStats(Ptr<const Packet> packet,
    ...
    {
      // 关键：从新版结构体中提取双精度浮点数
      double signalDbm = snr.signal;
      double noiseDbm = snr.noise;

      // --- 以下保持你原有的感知逻辑不变 ---
      // 例如：
      double currentSnr = signalDbm - noiseDbm;

      NS_LOG_DEBUG("SmartAODV Sensing - Packet Received: " << packet->GetUid()
    ...
    RoutingProtocol::PredictLinkExpiry(double rssi)
    {
      ...
      // Simple prediction model: higher RSSI = longer expected link lifetime
      // Thresholds can be adjusted based on simulation scenarios
      const double RSSI_HIGH_THRESHOLD = -60.0; // dBm
      const double RSSI_LOW_THRESHOLD = -85.0;  // dBm

      if (rssi >= RSSI_HIGH_THRESHOLD)
      {
        // Strong signal - link expected to be stable for a while
        return Seconds(5.0);
      }
      else if (rssi >= RSSI_LOW_THRESHOLD)
      {
        // Moderate signal - link may degrade soon
        return Seconds(2.0);
      }
      else
      {
        // Weak signal - link likely to fail very soon
        return Seconds(0.5);
      }
    }

    // SmartAodv: Send preemptive RREQ before link failure
    void
    RoutingProtocol::SendPreemptiveRreq(Ipv4Address destination)
    {
      NS_LOG_FUNCTION(this << destination);

      NS_LOG_INFO("SmartAodv: Sending preemptive RREQ to " << destination);

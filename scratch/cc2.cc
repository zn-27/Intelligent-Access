...
main(int argc, char* argv[])
{
     ...
     LogComponentEnable("MixedWireless", LOG_LEVEL_INFO);
     LogComponentEnable("UdpEchoClientApplication", LOG_LEVEL_INFO);
     LogComponentEnable("UdpEchoServerApplication", LOG_LEVEL_INFO);
     LogComponentEnable("OnOffApplication", LOG_LEVEL_INFO);    // 启用发送端应用日志
     LogComponentEnable("PacketSink", LOG_LEVEL_INFO);          // 启用接收端应用日志
     //Export NS_LOG = MixedWireless = info ;
    //日志系统设置
    NS_LOG_INFO("------------开始开始-----------");
    //
    ...
    // 在Application configuration之前添加全局路由表生成
...
//000000000000000000000
  ...
  Ptr<FlowMonitor> monitor = flowmon.InstallAll ();  // 监控所有节点
    //000000000000000000000
    ///////////////////////////////////////////////////////////////////////////
    //                                                                       //
    // Run simulation                                                        //
 ...
 // 7. 输出统计结果
 // 修正后的FlowMonitor统计输出部分
monitor->CheckForLostPackets();  // 检查丢包
...
for (auto &it : stats) {
    ...
    // 吞吐量（单位：Mbps）
    std::cout << "  Throughput: " << it.second.rxBytes * 8.0 / 
    ...
    // 丢包率
    std::cout << "  Packet Loss Ratio: " << (it.second.txPackets - it.second.rxPackets) / 

...
apAppD->SetStopTime(Seconds(simTime - 1.0));
    ...
    stack.Install(ApB);
    stack.Install(ApC);
    stack2.Install(ApD);  //作为纯无中心域的网关需要aodv/olsr协议栈
    ...
    Ptr<Ipv4> ipv4ApD = ApD.Get(0)->GetObject<Ipv4>();
    ipv4ApD->SetAttribute("IpForward", BooleanValue(true));
    ...
    // 域D主机默认路由（指向域D AP的IP：10.4.1.4）
    Ipv4Address apDGateway = ifApD.GetAddress(0);
    for (uint32_t i = 0; i < StaD.GetN(); ++i)
    {
        Ptr<Node> h = StaD.Get(i);
        Ptr<Ipv4> ipv4h = h->GetObject<Ipv4>();
        Ptr<Ipv4StaticRouting> staticRouting = staticRoutingHelper.GetStaticRouting(ipv4h);
        staticRouting->SetDefaultRoute(apDGateway, 1);
    }
...
//     // 添加Ping通信流: StaA[3] (10.1.1.4) -> StaB[1] (10.2.1.2)
// V4PingHelper pingHelper(ifB.GetAddress(1));  // Ping目标地址为10.2.1.2 (StaB[1])
// pingHelper.SetAttribute("Interval", TimeValue(Seconds(1.0)));  // 每秒发送一次ping
// pingHelper.SetAttribute("Size", UintegerValue(64));  // ping包大小
// pingHelper.SetAttribute("Verbose", BooleanValue(true));  // 显示ping结果

// ApplicationContainer pingApps = pingHelper.Install(StaA.Get(3));  // 在StaA[3]上安装ping应用
// pingApps.Start(Seconds(4.0));  // 4秒后开始ping
// pingApps.Stop(Seconds(simTime - 1));  // 在仿真结束前停止
    ...
    //调试信息输出：MAC地址和IP地址
    {
        ...
        // 输出域D的AP设备
        Ptr<NetDevice> devD = adhocDevsD1.Get(0);
        Address addrD = devD->GetAddress();
        std::cout << "  ApD[0] WiFi接口"
            << " -> MAC Address: "
            << Mac48Address::ConvertFrom(addrD);
        // 输出IP地址
        Ptr<Ipv4> ipv4D = ApD.Get(0)->GetObject<Ipv4>();
        if (ipv4D && ipv4D->GetNInterfaces() > 1) {
            std::cout << " -> IP Address: " << ipv4D->GetAddress(1, 0).GetLocal();
        }
        std::cout << std::endl;
    ...
    if (controllerApp != nullptr) {
        ...
        // 域D网关（AP）ARP条目
        Ipv4Address apDIp = ifApD.GetAddress(0);
        Mac48Address apDMac = Mac48Address::ConvertFrom(apCsmaDevsD.Get(0)->GetAddress());
        controllerApp->AddArpEntry(apDIp, apDMac);
        std::cout << "注册ARP条目：ApD[0] 网关 " << apDIp << " -> " << apDMac << std::endl;
    }

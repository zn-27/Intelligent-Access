...
//qlearning相关
// ========================= 全局变量和结构体（修改名称避免冲突：加C前缀）=========================
// 全局FlowMonitorHelper（只定义一次）
FlowMonitorHelper flowmonHelper;

// 性能指标存储
map<string, double> currentPerformanceMetrics;
map<string, double> previousPerformanceMetrics;

// 添加全局变量存储历史性能数据
map<uint32_t, FlowMonitor::FlowStats> previousFlowStats;
double previousTotalThroughput = 0.0;
double previousTotalLossRate = 0.0;
double previousAverageDelay = 0.0;

// 距离阈值配置（可调整）
const double DISTANCE_THRESHOLD = 25.0; // 平均节点距离阈值（米）

// 网络状态结构体（修改名称：NetworkState → CNetworkState）
struct CNetworkState {
    double averageNodeDistance; // 三维空间中节点间平均距离
    int nodeDensityLevel;        // 0-低密度（距离>阈值），1-高密度（距离<=阈值）
};

// 全局变量用于存储当前状态（使用修改后的结构体名称）
CNetworkState g_currentState;
string g_currentMode;

// ========================= Q-Learning 算法类（修改名称避免冲突：加C前缀）=========================
class CNetworkModeQLearning {
public:
    CNetworkModeQLearning(double alpha, double gamma, double epsilon)
        : alpha(alpha), gamma(gamma), epsilon(epsilon) {
        // 初始化 Q 表：2种状态 × 3个动作
        for (int i = 0; i < 2; ++i) {
            qTable[i] = vector<double>(3, 0.0); // 动作0:ADHOC, 1:MULTI, 2:KEEP
        }
    }

    // 将网络状态转换为状态ID（参数类型改为CNetworkState）
    int StateToId(const CNetworkState& state) {
        return state.nodeDensityLevel;
    }

    // 选择动作（参数类型改为CNetworkState）
    int ChooseAction(const CNetworkState& state) {
        int stateId = StateToId(state);
        int action;

        // 基于距离阈值的启发式决策
        if (state.averageNodeDistance <= DISTANCE_THRESHOLD) {
            action = (g_currentMode != "ADHOC") ? 0 : 2;
        } else {
            action = (g_currentMode != "MULTI") ? 1 : 2;
        }
        ...
        // 简化日志
        cout << "[QLearn] t=" << Simulator::Now().GetSeconds() << "s | 状态" << stateId 
              << " | 距离=" << state.averageNodeDistance << "m | 当前模式=" << g_currentMode 
              << " | 动作: " << (oldAction == 0 ? "ADHOC" : oldAction == 1 ? "MULTI" : "保持") 
              << (explored ? " [探索]" : "") << " → 最终动作=" 
              << (action == 0 ? "ADHOC" : action == 1 ? "MULTI" : "保持") << endl;

        return action;
    }

    // 更新 Q 表（参数类型改为CNetworkState）
    void Update(const CNetworkState& state, int action, const CNetworkState& newState, double reward) {
        int stateId = StateToId(state);
        int newStateId = StateToId(newState);
    
        double qPredict = qTable[stateId][action];
        double qTarget = reward + gamma * *max_element(qTable[newStateId].begin(), qTable[newStateId].end());
        qTable[stateId][action] += alpha * (qTarget - qPredict);
    
        // 简化日志
        cout << "[QLearn] 更新 | t=" << Simulator::Now().GetSeconds() << "s | 状态" << stateId 
              << "→动作" << action << "→状态" << newStateId << " | 奖励=" << reward 
              << " | Q值=" << fixed << setprecision(3) << qTable[stateId][action] << endl;
    }

    // 打印 Q 表
    void PrintQTable() {
        cout << "[QLearn] Q表 (状态0=低密度，状态1=高密度):" << endl;
        for (const auto& entry : qTable) {
            cout << "  状态" << entry.first << ": ADHOC=" << fixed << setprecision(3) << entry.second[0] 
                  << ", MULTI=" << entry.second[1] << ", 保持=" << entry.second[2] << endl;
        }
        cout << endl;
    }

private:
    map<int, vector<double>> qTable;
    double alpha, gamma, epsilon;
};

// 全局Q-Learning实例
CNetworkModeQLearning g_cGlobalRL(0.1, 0.9, 0.1);

// ========================= 辅助函数声明（修改参数/返回类型为CNetworkState）=========================
...
void EvaluateAndLearn(NodeContainer staNodes, NodeContainer apNodes, 
                    NetDeviceContainer staWifiDevs, NetDeviceContainer adhocDevs,
                    int action);
CNetworkState EvaluateNetworkState(NodeContainer staNodes); // 返回类型改为CNetworkState
double CalculateRewardBasedOnPerformance(map<FlowId, FlowMonitor::FlowStats> flowStats);
...
string DetectCurrentNetworkMode(NodeContainer staNodes, NetDeviceContainer staWifiDevs, NetDeviceContainer adhocDevs);
...
// 评估网络状态（返回类型改为CNetworkState）
CNetworkState EvaluateNetworkState(NodeContainer staNodes) {
    CNetworkState state;
    ...
    for (uint32_t i = 0; i < nodeCount; ++i) {
        ...
        for (uint32_t j = i + 1; j < nodeCount; ++j) {
            ...
            totalDistance += Calculate3dDistance(pos1, pos2);
            pairCount++;
        }
    }
    ...
    // 简化日志
    cout << "[QLearn] 网络状态 | 平均距离=" << state.averageNodeDistance << "m | " 
          << (state.nodeDensityLevel == 1 ? "高密度" : "低密度") << endl;
...
double CalculateRewardBasedOnPerformance(map<FlowId, FlowMonitor::FlowStats> flowStats) {
    ...
    for (auto const& flow : flowStats) {
        ...
        if (flow.second.txPackets > 0) {
            totalLossRate += (flow.second.txPackets - flow.second.rxPackets) / (double)flow.second.txPackets;
        }
        
        // 延迟
        if (flow.second.rxPackets > 0) {
            averageDelay += flow.second.delaySum.GetSeconds() / flow.second.rxPackets;
        }
    }
    ...
    double lossRateImprovement = previousTotalLossRate - totalLossRate;
    double delayImprovement = previousAverageDelay - averageDelay;
    ...
    // 计算奖励
    double reward = throughputImprovement * 0.3 + lossRateImprovement * 0.5 + delayImprovement * 0.2;
    
    // 简化日志
    cout << "[QLearn] 性能指标 | 吞吐量=" << fixed << setprecision(2) << totalThroughput/1000 << "Kbps | "
          << "丢包率=" << totalLossRate*100 << "% | 延迟=" << averageDelay*1000 << "ms | 奖励=" << reward << endl;
...
// 执行模式切换
void ExecuteGlobalSwitch(int action, NodeContainer staNodes, NetDeviceContainer staWifiDevs, NetDeviceContainer adhocDevs) {
    if (action == 0) { // 切换到ADHOC
        cout << "[QLearn] 执行切换: 所有节点→ADHOC模式" << endl;
        // 初始化控制器并调用方法
        // Ptr<OFSwitch13LearningController> controller = CreateObject<OFSwitch13LearningController>();
        // controller->CDL();//切换组网模式
        // controller->SetRoutingPriority();//切换路由协议
        for (uint32_t i = 0; i < staNodes.GetN(); ++i) {
            // DisableDeviceLogical(staNodes.Get(i), staWifiDevs.Get(i));
            EnableDeviceLogical(staNodes.Get(i), adhocDevs.Get(i));
        }
    } 
    else if (action == 1) { // 切换到MULTI
        cout << "[QLearn] 执行切换: 所有节点→MULTI模式" << endl;
    }
    else {
        cout << "[QLearn] 执行动作: 保持当前模式" << endl;
    }
}

// 评估并学习（修改所有CNetworkState相关引用）
void EvaluateAndLearn(NodeContainer staNodes, NodeContainer apNodes, 
                    NetDeviceContainer staWifiDevs, NetDeviceContainer adhocDevs,
                    int action)
{
    ...
    // 计算奖励
    double reward = CalculateRewardBasedOnPerformance(stats);
    
    // 保存历史统计
    previousFlowStats = stats;
    
    // 评估新状态（使用CNetworkState）
    CNetworkState newState = EvaluateNetworkState(staNodes);
    
    // 更新Q表（使用全局实例g_cGlobalRL）
    g_cGlobalRL.Update(g_currentState, action, newState, reward);
    
    // 定期打印Q表
    if (fmod(Simulator::Now().GetSeconds(), 5.0) < 1.0) {
        g_cGlobalRL.PrintQTable();
    }
}

// 全局周期模式切换（修改所有CNetworkState相关引用）
void GlobalPeriodicModeSwitch(NodeContainer staNodes, NodeContainer apNodes, 
                            NetDeviceContainer staWifiDevs, NetDeviceContainer adhocDevs)
{
    cout << "\n[QLearn] === 周期决策 (t=" << Simulator::Now().GetSeconds() << "s) ===" << endl;
    
    // 检测当前模式
    g_currentMode = DetectCurrentNetworkMode(staNodes, staWifiDevs, adhocDevs);
    cout << "[QLearn] 当前模式: " << g_currentMode << endl;
    
    // 评估网络状态（使用CNetworkState）
    g_currentState = EvaluateNetworkState(staNodes);
    
    // 选择动作（使用全局实例g_cGlobalRL）
    int action = g_cGlobalRL.ChooseAction(g_currentState);
    ...
    // 周期性调用
    Simulator::Schedule(Seconds(2.0), &GlobalPeriodicModeSwitch, staNodes, apNodes, staWifiDevs, adhocDevs);
}
...
void IntraDomainFlows(uint16_t basePort, uint16_t stop, NodeContainer& StaC, Ipv4InterfaceContainer& ifC, NetDeviceContainer& adhocDevsC)
{
    ...
    SendPacketByNodeId(StaC.Get(0), StaC.Get(1), ifC.GetAddress(4), port,
        "600kbps", 1024, 3.5, simTime - 1, adhocDevsC.Get(0), adhocDevsC.Get(1));
    ...
    SendPacketByNodeId(StaC.Get(1), StaC.Get(2), ifC.GetAddress(5), port + 2,
        "600kbps", 1024, 3.5, simTime - 1, adhocDevsC.Get(1), adhocDevsC.Get(2));
    SendPacketByNodeId(StaC.Get(1), StaC.Get(0), ifC.GetAddress(3), port + 3,
        "600kbps", 1024, 3.5, simTime - 1, adhocDevsC.Get(1), adhocDevsC.Get(0));
...
void InterDomainFlows(InterDomainParams* params)
{
    ...
    SendPacketByNodeId(params->StaC->Get(1), params->StaA->Get(2), params->ifA->GetAddress(2), port,
        "600kbps", 1024, 3.5, simTime - 1, params->staWifiDevsC->Get(1), params->staWifiDevsA->Get(2));
    SendPacketByNodeId(params->StaC->Get(2), params->StaA->Get(0), params->ifA->GetAddress(0), port + 1,
        "600kbps", 1024, 3.5, simTime - 1, params->staWifiDevsC->Get(2), params->staWifiDevsA->Get(0));
    SendPacketByNodeId(params->StaC->Get(2), params->StaB->Get(1), params->ifB->GetAddress(1), port + 2,
        "600kbps", 1024, 3.5, simTime - 1, params->staWifiDevsC->Get(2), params->staWifiDevsB->Get(1));
    SendPacketByNodeId(params->StaC->Get(1), params->StaB->Get(0), params->ifB->GetAddress(0), port + 3,
        "600kbps", 1024, 3.5, simTime - 1, params->staWifiDevsC->Get(1), params->staWifiDevsB->Get(0));
}
 ...
 int main(int argc, char* argv[])
{
    ...
    // //D域
    // Ptr<YansWifiChannel> channelD = channel.Create();
    // YansWifiPhyHelper phyD;
    // phyD.SetChannel(channelD);

    // mac.SetType("ns3::AdhocWifiMac");
    // NetDeviceContainer adhocDevsD1 = wifi.Install(phyD, mac, ApD);
    // NetDeviceContainer adhocDevsD2 = wifi.Install(phyD, mac, StaD);
...
for (uint32_t i = 0; i < StaC.GetN(); ++i) {
    Ptr<ApProtocolInfoApp> apOnStaApp = CreateObject<ApProtocolInfoApp>();
    StaC.Get(i)->AddApplication(apOnStaApp);
    apOnStaApp->SetStartTime(Seconds(0.0));
    apOnStaApp->SetStopTime(Seconds(simTime - 1.0));
}

// // 为域D的AP节点创建应用
// Ptr<ApProtocolInfoApp> apAppD = CreateObject<ApProtocolInfoApp>();
// ApD.Get(0)->AddApplication(apAppD);
// apAppD->SetStartTime(Seconds(0.0));
// apAppD->SetStopTime(Seconds(simTime - 1.0));
...
// }
    ...
    // // Flow1: StaA[3] -> StaB[1]
    // SendPacketByNodeId(StaA.Get(3), StaB.Get(1), ifB.GetAddress(1), port1,
    ...
    // // Flow2: StaB[0] -> StaC[1]
    // SendPacketByNodeId(StaC.Get(0), StaB.Get(0), ifB.GetAddress(0), port5,
    //     "600kbps", 1024, 3.5, simTime - 1, staWifiDevsC.Get(0), staWifiDevsB.Get(0));


    // Simulator::Schedule(Seconds(12.0), &IntraDomainFlows, 8000, simTime, StaC, ifC, adhocDevsC);
    // // 初始化参数结构体
    ...
    // );
        ...
        // 创建ping应用，测试StaA[1] -> StaC[1]的连通性
V4PingHelper pingHelper(ifC.GetAddress(1)); // StaC[2]的IP地址
pingHelper.SetAttribute("Verbose", BooleanValue(true));

// 在StaA[1]上安装ping应用
ApplicationContainer pingApps = pingHelper.Install(StaA.Get(1));
pingApps.Start(Seconds(3.0));   // 1秒后开始ping
pingApps.Stop(Seconds(simTime - 1.0));  // 在仿真结束前2秒停止

// // 测试StaA[2] -> StaC[2]
// V4PingHelper pingHelper3(ifC.GetAddress(2)); // StaC[2]的IP地址
// pingHelper3.SetAttribute("Verbose", BooleanValue(true));
// ApplicationContainer pingApps3 = pingHelper3.Install(StaA.Get(2));
// pingApps3.Start(Seconds(3.0));
// pingApps3.Stop(Seconds(simTime - 2.0));
    ...
    std::cout << std::endl;
        ...
        // //输出域D的Adhoc设备
        // for (uint32_t i = 0; i < StaD.GetN(); ++i) {
        //     Ptr<NetDevice> dev = adhocDevsD2.Get(i);
        //     Address addr = dev->GetAddress();
        //     std::cout << "  AdhocD[" << i << "] Adhoc接口"
        //         << " -> MAC Address: "
        //         << Mac48Address::ConvertFrom(addr);
        //     // 输出IP地址
        //     Ptr<Ipv4> ipv4 = StaD.Get(i)->GetObject<Ipv4>();
        //     if (ipv4 && ipv4->GetNInterfaces() > 1) {
        //         std::cout << " -> IP Address: " << ipv4->GetAddress(1, 0).GetLocal();
        //     }
        //     std::cout << std::endl;
...
if (controllerApp != nullptr) {
    ...
    // Mac48Address macD = Mac48Address::ConvertFrom(sw4Devsports.Get(0)->GetAddress());
    // std::cout << "Adding ARP entry: 10.4.1.254 -> " << macD << std::endl;
    // controllerApp->AddArpEntry(Ipv4Address("10.4.1.254"), macD); // 域D网关
}



// if (controllerApp != nullptr) {
//     // 为每个域添加ARP条目，使用网桥接口的MAC地址和IP地址
//     Mac48Address macA = Mac48Address::ConvertFrom(bridgeDevA.Get(0)->GetAddress());
//     Ipv4Address ipA = ifApA.GetAddress(0);  // 网桥A的IP地址
//     std::cout << "Adding ARP entry: " << ipA << " -> " << macA << std::endl;
//     controllerApp->AddArpEntry(ipA, macA);
    
//     Mac48Address macB = Mac48Address::ConvertFrom(bridgeDevB.Get(0)->GetAddress());
//     Ipv4Address ipB = ifApB.GetAddress(0);  // 网桥B的IP地址
//     std::cout << "Adding ARP entry: " << ipB << " -> " << macB << std::endl;
//     controllerApp->AddArpEntry(ipB, macB);
    
//     Mac48Address macC = Mac48Address::ConvertFrom(bridgeDevC.Get(0)->GetAddress());
//     Ipv4Address ipC = ifApC.GetAddress(0);  // 网桥C的IP地址
//     std::cout << "Adding ARP entry: " << ipC << " -> " << macC << std::endl;
//     controllerApp->AddArpEntry(ipC, macC);
// }
...
// Ptr<OFSwitch13Device> sw4Device = sw4->GetObject<OFSwitch13Device>();
...
// if (sw4Device) {
//     Simulator::Schedule(Seconds(3.0), &OFSwitch13Device::GetApStaMessages, sw4Device);
// }
...
// Simulator::Schedule(Seconds(3.0),&OFSwitch13Device::GetApStaMessages,sw3Device);
    ...
    // //组网模式切换
    // Simulator::Schedule(Seconds(5.0), &OFSwitch13LearningController::CDL, controllerApp);
    // //路由协议优先切换
    // Simulator::Schedule(Seconds(2.0), &OFSwitch13LearningController::SetRoutingPriority, controllerApp);

    // c域启用qlearning
    Simulator::Schedule(Seconds(3.0), &GlobalPeriodicModeSwitch, StaC, ApC, staWifiDevsC, adhocDevsC);

...
int main(int argc, char *argv[])
{
    ...
    onoff4.SetAttribute("StartTime", TimeValue(Seconds(3.5)));
    onoff4.SetAttribute("StopTime", TimeValue(Seconds(simTime - 1)));

    InetSocketAddress dst4(ifA.GetAddress(3), port1);
    onoff4.SetAttribute("Remote", AddressValue(dst4));

    ApplicationContainer app4 = onoff4.Install(StaB.Get(1));

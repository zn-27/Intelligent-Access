...
FlowMonitorHelper g_flowmonHelper; // 全局FlowMonitorHelper（仅用于Q-Learning）
// 距离阈值配置（可调整）
const double DISTANCE_THRESHOLD = 25.0; // 平均节点距离阈值（米）

// 网络状态结构体（NetworkState → CNetworkState）
...
public:
    CNetworkModeQLearning(double alpha, double gamma, double epsilon)
        ...
        // 初始化 Q 表：1种状态维度（基于距离阈值）× 3个动作
        // 状态直接用距离是否小于阈值判断，不再用ID映射
        qTable.resize(1, vector<double>(3, 0.0)); // 动作0:ADHOC, 1:MULTI, 2:KEEP
    }
    ...
    int ChooseAction(const CNetworkState& state) {
        ...
        // 简化日志
        cout << "[QLearn] t=" << Simulator::Now().GetSeconds() << "s | 距离=" << state.averageNodeDistance << "m | 当前模式=" << g_currentMode 
    ...
    void Update(const CNetworkState& state, int action, const CNetworkState& newState, double reward) {
        ...
        double qPredict = qTable[stateIdx][action];
        double qTarget = reward + gamma * *max_element(qTable[newStateIdx].begin(), qTable[newStateIdx].end());
        qTable[stateIdx][action] += alpha * (qTarget - qPredict);
    
        // 简化日志
        cout << "[QLearn] 更新 | t=" << Simulator::Now().GetSeconds() << "s | 动作" << action 
              << " | 奖励=" << reward << " | Q值=" << fixed << setprecision(3) << qTable[stateIdx][action] << endl;
    }

    // 打印 Q 表
    void PrintQTable() {
        cout << "[QLearn] Q表 (基于距离阈值决策):" << endl;
        cout << "  ADHOC=" << fixed << setprecision(3) << qTable[0][0] 
              << ", MULTI=" << qTable[0][1] << ", 保持=" << qTable[0][2] << endl;
        cout << endl;
    }

private:
    vector<vector<double>> qTable; // 改为vector，因只有1个状态维度
...
CNetworkState EvaluateNetworkState(NodeContainer staNodes) {
    ...
    // 简化日志
    cout << "[QLearn] 网络状态 | 平均距离=" << state.averageNodeDistance << "m" << endl;
...
// ========================= 全局配置 =========================
// 奖励函数权重系数
const double THROUGHPUT_WEIGHT = 0.5;    // 吞吐量权重（核心指标）
const double DELAY_WEIGHT = 0.2;         // 延迟权重（辅助指标）
const double LOSS_RATE_WEIGHT = 0.3;     // 丢包率权重（重要指标）

// 基础配置（复用原场景参数，不新增额外阈值）
const double MAX_THROUGHPUT = 600.0;     // 最大期望吞吐量（Kbps），与发送速率一致
const double MAX_DELAY = 100.0;          // 最大可接受延迟（ms）

// ========================= 奖励计算函数 =========================
// 计算奖励（仅监控域C流量）
double CalculateRewardBasedOnPerformance(Ptr<FlowMonitor> monitor, NodeContainer staCNodes) {
    ...
    // 提取域C节点的IP地址集合
    set<Ipv4Address> cNodeIps;
    for (uint32_t i = 0; i < staCNodes.GetN(); ++i) {
        Ptr<Ipv4> ipv4 = staCNodes.Get(i)->GetObject<Ipv4>();
        for (uint32_t j = 0; j < ipv4->GetNInterfaces(); ++j) {
            cNodeIps.insert(ipv4->GetAddress(j, 0).GetLocal());
        }
    }

    // 遍历所有流，仅统计域C相关的流（源或目的在域C）
    for (auto const& flow : stats) {
        FlowMonitor::FlowStats flowStats = flow.second;
        Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow(flow.first);

        // 过滤：仅保留源或目的在域C的流
        bool isCFlow = (cNodeIps.count(t.sourceAddress) > 0) || (cNodeIps.count(t.destinationAddress) > 0);
        if (!isCFlow) {
            continue; // 跳过非域C流量
        }
        ...
        double throughput = (flowStats.rxBytes * 8.0) / (duration * 1024.0);
        ...
        // 3. 丢包率
        double lossRate = 1.0 - (static_cast<double>(flowStats.rxPackets) / flowStats.txPackets);
        totalLossRate += lossRate;

        // 域C流量日志
        cout << "[QLearn] 域C流量 | 流(" << t.sourceAddress << "→" << t.destinationAddress 
             << ") | 吞吐量=" << throughput << "Kbps | 延迟=" << delay << "ms | 丢包率=" << lossRate << endl;
    }

    // 处理无有效流的情况
    if (validFlowCount == 0) {
        cout << "[QLearn] 无域C有效流，奖励值为0" << endl;
        return 0.0;
    }

    // 计算平均指标
    double avgThroughput = totalThroughput / validFlowCount;
    double avgDelay = totalDelay / validFlowCount;
    double avgLossRate = totalLossRate / validFlowCount;

    // 计算各指标贡献
    double throughputContribution = THROUGHPUT_WEIGHT * (avgThroughput / MAX_THROUGHPUT);
    throughputContribution = min(throughputContribution, THROUGHPUT_WEIGHT);

    double delayContribution = -DELAY_WEIGHT * (avgDelay / MAX_DELAY);
    delayContribution = max(delayContribution, -DELAY_WEIGHT);

    double lossRateContribution = -LOSS_RATE_WEIGHT * avgLossRate;
    lossRateContribution = max(lossRateContribution, -LOSS_RATE_WEIGHT);

    // 最终奖励（最小为0）
    double reward = throughputContribution + delayContribution + lossRateContribution;
    reward = max(reward, 0.0);

    // 奖励详情日志
    cout << "[QLearn] 奖励计算详情:" << endl;
    cout << "  域C有效流数量: " << validFlowCount << endl;
    cout << "  平均吞吐量: " << fixed << setprecision(2) << avgThroughput << " Kbps (贡献: " << throughputContribution << ")" << endl;
    cout << "  平均延迟:   " << fixed << setprecision(2) << avgDelay << " ms (贡献: " << delayContribution << ")" << endl;
    cout << "  平均丢包率: " << fixed << setprecision(3) << avgLossRate << " (贡献: " << lossRateContribution << ")" << endl;
    cout << "  奖励值:     " << fixed << setprecision(3) << reward << endl;
...
void EvaluateAndLearn(NodeContainer staNodes, NodeContainer apNodes, 
                    ...
                    int action)
{
     // 获取FlowMonitor实例（仅监控域C）
    Ptr<FlowMonitor> monitor = g_flowmonHelper.GetMonitor();
    
    // 计算奖励（仅基于域C流量）
    double reward = CalculateRewardBasedOnPerformance(monitor, staNodes);
...
void MonitorFlow(Ptr<FlowMonitor> monitor, FlowMonitorHelper* flowHelper, double interval, std::ofstream* fout)
{
    ...
    *fout << now;
    ...
    for (auto port : ports)
    {
        ...
        // std::cout << "链路通信数据Port " << port << ": Throughput=" << throughput
        //           << " Kbps, LossRate=" << lossRate
        //           << " %, AvgRTT=" << avgRtt
 ...
 int main(int argc, char* argv[])
{
    ...
    // NetDeviceContainer apOnStaDevsC = wifi.Install(phyC, mac, StaC);// 关键：在STA节点上创建AP接口
...
for (uint32_t i = 0; i < StaC.GetN(); ++i) {
    ...
    // Flow0: StaA[1] -> StaC[1]
    SendPacketByNodeId(StaA.Get(1), StaC.Get(1), ifC.GetAddress(1), port0,
        "600kbps", 1024, 3.5, simTime - 1, staWifiDevsA.Get(1), staWifiDevsC.Get(1));
...
// pingApps.Start(Seconds(1.0));   // 1秒后开始ping
// pingApps.Stop(Seconds(simTime - 2.0));  // 在仿真结束前2秒停止
...
// ========================= 启用Q-Learning（仅域C）=========================
    // 安装Q-Learning专用的FlowMonitor（仅监控域C节点）
    // Ptr<FlowMonitor> qlMonitor = g_flowmonHelper.Install(StaC);
    NodeContainer staNodes;
    staNodes.Add(StaA);
    staNodes.Add(StaB);
    staNodes.Add(StaC);
    Ptr<FlowMonitor> qlMonitor = g_flowmonHelper.Install(staNodes);
    // 启动Q-Learning周期决策（3秒启动）

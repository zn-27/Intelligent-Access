...
main (int argc, char *argv[])
{
  ...
  NodeContainer hosts;
  hosts.Create (2);
  // Ptr<ConstantPositionMobilityModel> h1_m = hosts.Get(0)->GetObject<ConstantPositionMobilityModel>();
  // h1_m->SetPosition(Vector(-distance, 0.0, 0.0));
  ...
  // ====================创建移动性助手==========================
    ...
    positionAlloc->Add (Vector (-10.0, 0.0, 0.0));   // 节点0的位置 (x, y, z)
    positionAlloc->Add (Vector (10.0, 0.0, 0.0));  // 节点1的位置
    positionAlloc->Add (Vector (0.0, -10.0, 0.0)); // 节点2的位置
    positionAlloc->Add (Vector (0.0, -20.0, 0.0)); // 节点3的位置
    // 可以继续添加更多节点的位置
    // 将位置分配器关联到移动性助手
    ...
    mobility.Install (hosts);  // nodes是你创建的节点容器

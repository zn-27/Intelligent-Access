...
namespace ns3
{
      ...
      uint32_t xid)
  {
    ...
    if (reason == OFPR_NO_MATCH)
    {
      ...
      // 检查是不是 IPv4 包
      struct ofl_match_tlv *ethType =
          oxm_match_lookup(OXM_OF_ETH_TYPE, (struct ofl_match *)msg->match);
      if (ethType)
      {
        uint16_t type;
        memcpy(&type, ethType->value, 2);
        if (ntohs(type) == 0x0800) // IPv4
        {
          // 如果你不想做 IP 学习，直接 flood 未知 IP
          outPort = OFPP_FLOOD;
          NS_LOG_DEBUG("Unknown IPv4 packet, flooding");
        }
      }

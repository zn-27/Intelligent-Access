...
OFSwitch13Device::CollectFlowStats()
{
  ...
  int OFSwitch13Device::SendOpenflowBufferToRemote(struct ofpbuf *buffer,
                                        ...
                                        uint64_t cookie)
  {
    ...
    case (OFPP_CONTROLLER):
    {
      dev->SendPacketInMessage(pkt, pkt->table_id,
                               pkt->handle_std->table_miss ? OFPR_NO_MATCH : OFPR_ACTION, maxLength, cookie);
      break;
    }
    case (OFPP_FLOOD):
    case (OFPP_ALL):
    {
      ...
      {
        if ((p->stats->port_no == pkt->in_port) || (outPort == OFPP_FLOOD && p->conf->config & OFPPC_NO_FWD))
        {
          continue;
        }
  ...
  OFSwitch13Device::DatapathTimeout(struct datapath *dp)
  {
    ...
    m_pipeDelay = m_sumFlowEntries < 2U ? m_tcamDelay : m_tcamDelay * (int64_t)ceil(log2(m_sumFlowEntries));
  ...
  int OFSwitch13Device::SendPacketInMessage(struct packet *pkt, uint8_t tableId,
  ...
  int OFSwitch13Device::SendToController(Ptr<Packet> packet,
  ...
  int OFSwitch13Device::ReplyWithErrorMessage(ofl_err error, struct ofpbuf *buffer,
                                              struct sender *senderCtrl)
  {
  ...
  OFSwitch13Device::NotifyPacketDestroyed(struct packet *pkt)
  {
    ...
    NS_ASSERT_MSG((m_pipePkt.IsValid() && !m_pipePkt.HasId(pkt->ns3_uid)) || !m_pipePkt.IsValid(), "Packet still valid in pipeline.");
                                               ...
                                               struct flow_table *table)
  {
    ...
    NS_ASSERT_MSG(m_pipePkt.HasId(pkt->ns3_uid), "Invalid packet ID.");
    NS_LOG_DEBUG("OpenFlow table id " << static_cast<uint16_t>(tableId) << " dropped unmatched packet " << pkt->ns3_uid);
